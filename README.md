...
機械学習研究に必須の公式ライブラリの実用のための要点を日本語で説明するmkdocs

## 🏗️ 構造マップ（全体像）

```
./
├── benchmarks/           # 実験統合スクリプト (run_all, hpo_sweep, etc.)
├── core/                 # 主要アルゴリズム (Diffusion, CrossAttention)
├── utils/                # 評価・統計・可視化・データ前処理
├── conf/                 # Hydra 設定ファイル群
└── mlruns/               # MLflow 実験ログ
```

各層には、次のような技術が連携しています：
